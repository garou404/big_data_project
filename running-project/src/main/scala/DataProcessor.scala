...
object DataProcessor {
    def get_runner_proportion_per_country(df_runner : DataFrame, df_pop : DataFrame) : DataFrame = {
        ...
        .withColumn("runner_proportion", expr("runner_nb_per_country / population"))
        .withColumn("runner_proportion", round(col("runner_proportion"), 6))
        .sort(desc("runner_proportion"))
        .drop("runner_nb_per_country", "population")
    }
    ...
    def get_fastest_runners_for_distance(df_runner : DataFrame, distance_threshold : Double, pace_threshold : Double, gender : String, limit : Int) : DataFrame = {
    ...
    def get_all_time_performances(df_runner : DataFrame, df_wr : DataFrame, limit : Int) : DataFrame = {
        ...
        .flatMap(distance => genders.map(gender => {
            ...
            get_fastest_runners_for_distance(temp_df, distance, wr.asInstanceOf[Double], gender, limit)
        }))      
        .reduce(_ union _)
        df_selected
    }
